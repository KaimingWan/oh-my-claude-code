...
|-------|-----------|-----------|
| L1 Commands | `/plan` `/debug` `/research` `/review` `/skill` | 100% — user triggers full workflow |
...
│  /plan · /debug · /research · /review · /skill           │
...
│  context-enrichment (correction detection + lessons)     │
├─────────────────────────────────────────────────────────┤
...
| Command | Workflow |
|---------|----------|
| `/plan` | brainstorming → write plan → reviewer challenge → fix until APPROVE → user confirm → execute |
...
| `feedback/verify-completion.sh` | Stop hook: completion criteria check → LLM eval → correction feedback |
...
| `reviewer` | Plan & code review | read, shell | Read-only, must cite file:line |
...
│   └── feedback/                  # auto-test, auto-lint, verify-completion, context-enrichment
├── skills/                        # 9 core skills
├── agents/                        # Subagent prompt files
├── commands/                      # Custom commands (plan, debug, research, review, skill)
├── scripts/
...
│   └── rules/                     # security.md, code-quality.md, git-workflow.md
