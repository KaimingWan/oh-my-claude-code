...
- [ ] detect_cli 在循环外调用 | `python3 -c "s=open('scripts/ralph_loop.py').read(); l=s.index('for i in range(1,'); print('PASS' if 'detect_cli()' not in s[l:] else 'FAIL')" | grep -q PASS`
- [ ] precheck 只在首次迭代跑 | `python3 -c "s=open('scripts/ralph_loop.py').read(); b=s.split('def build_prompt(')[1].split('\ndef ')[0]; print('PASS' if 'run_precheck' not in b else 'FAIL')" | grep -q PASS`
- [ ] build_init_prompt 已合并 | `python3 -c "s=open('scripts/ralph_loop.py').read(); print('PASS' if 'def build_init_prompt(' not in s else 'FAIL')" | grep -q PASS`
